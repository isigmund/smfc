...
#           ipmitool_cmd = [self.command, 'raw', '0x30', '0x70', '0x66', '0x01', str(zone), str(level) ]
           ipmitool_cmd1 = [self.command, 'raw', '0x30', '0x70', '0x66', '0x01', str(zone), str(level) ]
           ipmitool_cmd2 = [self.command, 'raw', '0x30', '0x70', '0x66', '0x01', str(zone), str(level) ]
        elif self.ipmitool_raw_mode == self.RAW_MODE_HEX:
#           ipmitool_cmd = [self.command, 'raw', '0x30', '0x70', '0x66', '0x01', str(hex(zone)), str(hex(level)) ]
           ipmitool_cmd1 = [self.command, 'raw', '0x30', '0x70', '0x66', '0x01', str(hex(zone)), str(hex(level)) ]
           ipmitool_cmd2 = [self.command, 'raw', '0x30', '0x70', '0x66', '0x01', str(hex(zone)), str(hex(level)) ]
        else:
           raise ValueError(f'Invalid IPMITOOL raw mode({self.ipmitool_raw_mode}).')
        # Set the new IPMI fan level in the specific zone
        try:
#              subprocess.run(ipmitool_cmd, check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
              subprocess.run(ipmitool1_cmd, check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
              subprocess.run(ipmitool2_cmd, check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except FileNotFoundError as e:
            raise e
